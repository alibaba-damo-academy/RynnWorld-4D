...
def get_latest_ckpt_path_to_resume_from(
    resume_from_checkpoint: str | None, num_update_steps_per_epoch: int, output_dir: str | None = None
    ...
    if resume_from_checkpoint is None:
        ...
        if output_dir is None or not os.path.exists(output_dir):
            logger.info("output_dir not provided or does not exist. Starting a new training run.")
            initial_global_step = 0
            global_step = 0
            first_epoch = 0
            resume_from_checkpoint_path = None
            ...
            for d in os.listdir(output_dir):
                ...
                if os.path.isdir(full_path) and d.startswith("checkpoint-"):
                    try:
                        ...
                        continue
            if not ckpt_dirs:
                logger.info("No checkpoints found in output_dir. Starting a new training run.")
                initial_global_step = 0
                global_step = 0
                first_epoch = 0
                resume_from_checkpoint_path = None
